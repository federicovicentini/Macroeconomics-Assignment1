...
First of all, we need to get the data and we needed from the Fred database, using a for cycle and the function \textttt{getSymbols()}
...
Subsequently we create new vectors for list of the two countries selected, for NIMA aggregates (Investment, Consumption, Import, Export, Public Spending) and to transform the name for columns in the matrix dataeu
```{r p1.6}
...
# First of all we import the library BCDating 
library(BCDating)

# We choose to using data in log level. We generate a matrix to fill with the log of the variables
data_bc <- matrix(NA, length(GDP), 4)

# Taking logs to trasform data
data_bc[, 1] <- log(GDP)
data_bc[, 2] <- log(IMPGS)
data_bc[, 3] <- log(GPDI)
data_bc[, 4] <- log(PCEC)
